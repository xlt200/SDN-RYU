...
class shortest_path(app_manager.RyuApp):
        ...
        def packet_in_handler(self, ev):
                ...
                if pkt_ethernet.dst == 'ff:ff:ff:ff:ff:ff':
                        if self.mac_learning(dp,pkt_ethernet.src,port) is False: #判断是否会造成广播风暴，False代表為是會造成
                                ...
                                if not self.net.has_node(pkt_ethernet.src): #若arp封包源沒有包含在圖裡面，則將它加入進圖中
                                        ...
                                        self.net.add_edge(pkt_ethernet.src,dp.id,bw=0,latency=0, lldpdelay=0) #添加雙向的邊
                                        self.net.add_edge(dp.id,pkt_ethernet.src,port=port,bw=0, latency=0, lldpdelay=0)
                ...
                if pkt_ethernet.ethertype != 34525 and pkt_ethernet.ethertype != 35020:
                        if not self.net.has_node(pkt_ethernet.src): #如果不在图里则新增
                                ...
                                self.net.add_node(pkt_ethernet.src)
                                self.net.add_edge(pkt_ethernet.src, dp.id, bw=0, latency=0, lldpdelay=0)
        ...
        def get_topology_data(self, ev):
                time.sleep(1)# 等待拓扑建立完成
                switch_list = get_switch(self.topology_api_app, None)
                switches =[switch.dp.id for switch in switch_list]
                links_list = get_link(self.topology_api_app, None)
                links=[(link.src.dpid,link.dst.dpid,{'port':link.src.port_no,'bw':0,'latency':0, 'lldpdelay':0}) for link in links_list] 
        ...
        def port_stats_event_handler(self, ev):
                ...
                for l in range(length):
                    ...
                    src_dpid = edgels[l][0]
                    dst_dpid = edgels[l][1]
                    print("(link {} -> {} bw: {} latency: {} lldpdelay: {})".format(src_dpid,dst_dpid,self.net[src_dpid][dst_dpid]["bw"],self.net[src_dpid][dst_dpid]["latency"],self.net[src_dpid][dst_dpid]["lldpdelay"]))
            ...
            for src_dpid in self.net:
                for dst_dpid in self.net[src_dpid]:
                    if src_dpid == dst_dpid:
                        self.net[src_dpid][dst_dpid]['latency'] = 0
                        continue
                    delay = self.get_delay(src_dpid, dst_dpid)
                    if delay != float('inf'):
                        self.net[src_dpid][dst_dpid]['latency'] = delay
            return
